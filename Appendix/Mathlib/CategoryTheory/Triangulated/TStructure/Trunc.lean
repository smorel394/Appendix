...
import Appendix.Mathlib.CategoryTheory.Triangulated.TStructure.Basic
...
lemma triangle_map_ext' (a b : ℤ) (hab : a ≤ b) {T T' : Triangle C} (f₁ f₂ : T ⟶ T')
  ...
  suffices ∀ (f : T ⟶ T') (_ : f.hom₂ = 0), f = 0 by
    ...
    rw [H, sub_self]
  intro f hf
  have : t.IsGE T'.invRotate.obj₁ (b + 1) := by
    dsimp
    refine {ge := ?_}
    change (t.ge (b + 1)).shift (-1) _
    rw [t.shift_ge (-1) (b + 1) b (by omega)]
    exact t.ge_of_isGE _ _
  have : t.IsLE ((T.obj₁)⟦(1 : ℤ)⟧) (a - 1) := by
    refine {le := ?_}
    change (t.le (a - 1)).shift 1 _
    rw [t.shift_le 1 (a - 1) a (by omega)]
  ...
  · obtain ⟨g, hg⟩ := Triangle.coyoneda_exact₂ _ (inv_rot_of_distTriang _ hT') f.hom₁ (by
      ...
      dsimp at eq ⊢
      rw [← eq, hf, comp_zero])
    have hg' : g = 0 := t.zero_of_isLE_of_isGE g a (b+1) (by omega)
    simp [hg, hg']
  · simp [hf]
  · obtain ⟨g, hg⟩ := T.yoneda_exact₃ hT f.hom₃ (by rw [f.comm₂, hf, zero_comp])
    have hg' : g = 0 := t.zero_of_isLE_of_isGE g (a-1) b (by omega)
    simp [hg, hg']
...
lemma triangle_iso_exists (n₀ n₁ : ℤ) (h : n₀ < n₁) (T T' : Triangle C)
  ...
  exact ⟨
    { hom := hom
      inv := inv
      hom_inv_id := triangle_map_ext' t n₀ n₁ (by omega) _ _ hT hT h₀ h₁
        (by simp only [comp_hom₂, id_hom₂, hhom, hinv, e.hom_inv_id])
      inv_hom_id := triangle_map_ext' t n₀ n₁ (by omega) _ _ hT' hT' h₀' h₁'
...
noncomputable def triangle : Triangle C :=
  Triangle.mk
    (t.exists_triangle A (n-1) n
      (by omega)).choose_spec.choose_spec.choose_spec.choose_spec.choose
    (t.exists_triangle A (n-1) n
      (by omega)).choose_spec.choose_spec.choose_spec.choose_spec.choose_spec.choose
    (t.exists_triangle A (n-1) n
      (by omega)).choose_spec.choose_spec.choose_spec.choose_spec.choose_spec.choose_spec.choose

lemma triangle_distinguished :
    triangle t n A ∈ distTriang C :=
  (t.exists_triangle A (n-1) n (by omega)
...
instance triangle_obj₁_isLE (n : ℤ) :
  ...
  exact ⟨(t.exists_triangle A (n-1) n (by omega)).choose_spec.choose_spec.choose⟩
...
instance triangle_obj₃_isGE :
  ...
  ⟨(t.exists_triangle A (n-1) n (by omega)).choose_spec.choose_spec.choose_spec.choose⟩
...
lemma triangle_map_ext (f₁ f₂ : triangle t n A ⟶ triangle t n B)
    (H : f₁.hom₂ = f₂.hom₂) : f₁ = f₂ :=
  triangle_map_ext' t (n-1) n (by omega) _ _
...
noncomputable def triangleMap : triangle t n A ⟶ triangle t n B :=
  have H := triangle_map_exists t (n-1) n (by omega) _ _ (triangle_distinguished t n A)
...
noncomputable def triangleMapOfLE (a b : ℤ) (h : a ≤ b) : triangle t a A ⟶ triangle t b A :=
  have H := triangle_map_exists t (a-1) b (by omega) _ _ (triangle_distinguished t a A)
...
noncomputable def triangleFunctorNatTransOfLE (a b : ℤ) (h : a ≤ b) :
  ...
  naturality {X₁ X₂} φ := by
    refine triangle_map_ext' t (a-1) b (by omega) _ _
...
lemma triangleFunctorNatTransOfLE_trans (a b c : ℤ) (hab : a ≤ b) (hbc : b ≤ c) :
  ...
  apply NatTrans.ext
  ext1 X
  exact triangle_map_ext' t (a-1) c (by omega) _ _
...
lemma triangleFunctorNatTransOfLE_refl (a : ℤ) :
  ...
  apply NatTrans.ext
  ext1 X
  exact triangle_map_ext' t (a-1) a (by omega) _ _
...
instance (n : ℤ) (X : C) : t.IsLE ((t.truncLE n).obj X) n := by
  have : t.IsLE ((t.truncLE n).obj X) (n+1-1) := by
    ...
    infer_instance
  refine {le := ?_}
  apply t.le_monotone (b := n) (a := n + 1 - 1) (by omega)
  exact t.le_of_isLE _ _
...
instance (n : ℤ) (X : C) : t.IsGE ((t.truncGT (n-1)).obj X) n := by
  refine {ge := ?_}
  apply t.ge_antitone (a := n) (b := n - 1 + 1) (by omega)
  exact t.ge_of_isGE _ _
...
noncomputable def truncGTδLE (n : ℤ) :
  ...
  (t.truncGTIsoTruncGE n (n+1) rfl).hom ≫ t.truncGEδLE n (n+1) (by omega)
...
lemma isZero_truncLE_obj_zero (n : ℤ) : IsZero ((t.truncLE n).obj 0) := by
  ...
  have : t.IsLE ((t.truncLE n ⋙ shiftFunctor C (1 : ℤ)).obj 0) (n-1) := by
    dsimp
    refine {le := ?_}
    change (t.le _).shift _ _
    rw [t.shift_le 1 (n - 1) n (by omega)]
    exact t.le_of_isLE _ _
  have : t.IsLE ((t.truncGE (n + 1)).obj 0) (n - 1) :=
    {le := (t.le (n - 1)).prop_of_iso (asIso δ).symm (t.le_of_isLE _ _)   }
  have : t.IsGE ((t.truncLE n ⋙ shiftFunctor C (1 : ℤ)).obj 0) (n + 1) :=
    {ge := (t.ge (n + 1)).prop_of_iso (asIso δ) (t.ge_of_isGE _ _)}
  have hδ := t.zero_of_isLE_of_isGE δ (n-1) (n+1) (by omega)
...
lemma isZero_truncGE_obj_zero (n : ℤ) : IsZero ((t.truncGE n).obj 0) := by
  apply (Triangle.isZero₃_iff_isIso₁ _ (t.triangleLEGE_distinguished (n-1) n (by omega) 0)).2
  exact ⟨⟨0, (isZero_truncLE_obj_zero t (n-1)).eq_of_src _ _, (isZero_zero _).eq_of_src _ _⟩⟩

instance (n : ℤ) : t.IsLE (0 : C) n :=
  {le := (t.le n).prop_of_iso (t.isZero_truncLE_obj_zero n).isoZero (t.le_of_isLE _ _)}

instance (n : ℤ) : t.IsGE (0 : C) n :=
  {ge := (t.ge n).prop_of_iso (t.isZero_truncGE_obj_zero n).isoZero (t.ge_of_isGE _ _)}

lemma isLE_of_isZero (X : C) (hX : IsZero X) (n : ℤ) : t.IsLE X n :=
  {le := (t.le n).prop_of_iso hX.isoZero.symm (t.le_of_isLE _ _)}

lemma isGE_of_isZero (X : C) (hX : IsZero X) (n : ℤ) : t.IsGE X n :=
  {ge := (t.ge n).prop_of_iso hX.isoZero.symm (t.ge_of_isGE _ _)}

lemma isLE_iff_isIso_truncLEι_app (n : ℤ) (X : C) :
    t.IsLE X n ↔ IsIso ((t.truncLEι n).app X) := by
  constructor
  · intro
    obtain ⟨e, he⟩ := t.triangle_iso_exists n (n+1) (by omega) _ _
    ...
    infer_instance
  · intro
    exact {le := (t.le n).prop_of_iso (asIso ((truncLEι t n).app X)) (t.le_of_isLE _ _)}
...
lemma isGE_iff_isIso_truncGEπ_app (n : ℤ) (X : C) :
  ...
  constructor
  · intro h
    obtain ⟨e, he⟩ := t.triangle_iso_exists (n-1) n (by omega) _ _
      (inv_rot_of_distTriang _ (contractible_distinguished X))
      (t.triangleLTGE_distinguished n X) (Iso.refl X)
      {le := (t.le (n - 1)).prop_of_iso (shiftFunctor C (-1 : ℤ)).mapZeroObject.symm
             (t.le_of_isLE _ _)}
      (by dsimp ; infer_instance) (by dsimp ; infer_instance) (by dsimp ; infer_instance)
    dsimp at he
    ...
    infer_instance
  · intro
    exact {ge := (t.ge n).prop_of_iso (asIso ((truncGEπ t n).app X)).symm (t.ge_of_isGE _ _)}
...
lemma from_truncGE_obj_ext (n : ℤ) (X : C) {Y : C}
  ...
  have : t.IsLE ((t.triangleLTGE n).obj X).obj₁ (n - 1) := by
    dsimp; infer_instance
  have : t.IsLE (((t.triangleLTGE n).obj X).obj₁⟦(1 : ℤ)⟧) (n - 2) := by
    refine {le := ?_}
    change (t.le (n - 2)).shift _ _
    rw [t.shift_le 1 (n - 2) (n - 1)  (by omega)]
    exact t.le_of_isLE _ _
  have hg' := t.zero_of_isLE_of_isGE g (n-2) n (by omega)
  rw [hg, hg', comp_zero]
...
lemma to_truncLE_obj_ext (n : ℤ) (Y : C) {X : C}
  ...
  have : t.IsGE ((t.triangleLEGT n).obj X).invRotate.obj₁ (n + 2) := by
    dsimp
    refine {ge := ?_}
    change (t.ge _).shift _ _
    rw [t.shift_ge _ _ (n + 1) (by omega)]
    exact t.ge_of_isGE _ _
  have hg' := t.zero_of_isLE_of_isGE g n (n+2) (by omega)
  rw [hg, hg', zero_comp]
...
lemma to_truncLT_obj_ext (n : ℤ) (Y : C) {X : C}
    ...
    [t.IsLE Y (n-1)] :
    f₁ = f₂ := by
  rw [← cancel_mono ((t.truncLEIsoTruncLT (n-1) n (by omega)).inv.app X)]
...
lemma liftTruncLE' {X Y : C} (f : X ⟶ Y) (n : ℤ) [t.IsLE X n] :
  ...
  have : t.IsGE ((t.triangleLEGT n).obj Y).obj₃ (n + 1) := by dsimp; infer_instance
  Triangle.coyoneda_exact₂ _ (t.triangleLEGT_distinguished n Y) f
    (t.zero_of_isLE_of_isGE  _ n (n+1) (by omega))
...
lemma descTruncGE' {X Y : C} (f : X ⟶ Y) (n : ℤ) [t.IsGE Y n] :
  ...
  have : t.IsLE ((t.triangleLTGE n).obj X).obj₁ (n - 1) := by dsimp; infer_instance
  Triangle.yoneda_exact₂ _ (t.triangleLTGE_distinguished n X) f
    (t.zero_of_isLE_of_isGE _ (n-1)  n (by omega))
...
lemma isLE₂ (T : Triangle C) (hT : T ∈ distTriang C) (n : ℤ) (h₁ : t.IsLE T.obj₁ n)
  ...
  obtain ⟨f', hf'⟩ := Triangle.yoneda_exact₂ _ hT f
    (t.zero _ n (n+1) (by omega) )
  rw [hf', t.zero f' n (n+1) (by omega), comp_zero]
...
lemma isGE₂ (T : Triangle C) (hT : T ∈ distTriang C) (n : ℤ) (h₁ : t.IsGE T.obj₁ n)
  ...
  intro Y f hY
  obtain ⟨f', hf'⟩ := Triangle.coyoneda_exact₂ _ hT f (t.zero _ (n-1) n (by omega))
  rw [hf', t.zero f' (n-1) n (by omega), zero_comp]
...
def minus : Triangulated.Subcategory C := Triangulated.Subcategory.mk'
  ...
  (fun X n => by
    ...
    refine ⟨i - n, {le := ?_}⟩
    change (t.le _).shift _ _
    rw [t.shift_le _ _ i (by omega)]
    exact t.le_of_isLE _ _)
  (fun T hT => by
    rintro ⟨i₁, hi₁⟩ ⟨i₃, hi₃⟩
    refine ⟨max i₁ i₃, ?_⟩
    refine t.isLE₂ T hT _ {le := ?_} {le := ?_}
    · exact t.le_monotone (a := i₁) (le_max_left _ _) _ (t.le_of_isLE _ _)
    · exact t.le_monotone (a := i₃) (le_max_right _ _) _ (t.le_of_isLE _ _))

open ObjectProperty

instance : IsClosedUnderIsomorphisms t.minus.P := by
  dsimp only [minus]
  infer_instance
...
def plus : Triangulated.Subcategory C := Triangulated.Subcategory.mk'
  ...
  (fun X n => by
    ...
    refine ⟨i - n, {ge := ?_}⟩
    change (t.ge _).shift _ _
    rw [t.shift_ge _ _ i (by omega)]
    exact t.ge_of_isGE _ _)
  (fun T hT => by
    rintro ⟨i₁, hi₁⟩ ⟨i₃, hi₃⟩
    refine ⟨min i₁ i₃, ?_⟩
    refine t.isGE₂ T hT _ {ge := ?_} {ge := ?_}
    · exact t.ge_antitone (b := i₁) (min_le_left _ _) _ (t.ge_of_isGE _ _)
    · exact t.ge_antitone (b := i₃) (min_le_right _ _) _ (t.ge_of_isGE _ _))

instance : IsClosedUnderIsomorphisms t.plus.P := by
  dsimp only [plus]
  infer_instance

def bounded : Triangulated.Subcategory C := t.plus.inter t.minus

instance : IsClosedUnderIsomorphisms t.bounded.P := by
  dsimp [bounded]
  infer_instance

noncomputable def natTransTruncLEOfLE (a b : ℤ) (h : a ≤ b) :
    t.truncLE a ⟶ t.truncLE b :=
  t.natTransTruncLTOfLE (a+1) (b+1) (by omega)
...
lemma isIso_truncLTmap_iff {X Y : C} (f : X ⟶ Y) (n : ℤ) :
  ...
  · rintro ⟨Z, g, h, mem, _⟩
    obtain ⟨e, he⟩ := t.triangle_iso_exists (n-1) n (by omega)  _ _ mem
...
lemma isIso_truncGEmap_iff {Y Z : C} (g : Y ⟶ Z) (n₀ n₁ : ℤ) (hn₁ : n₀ + 1 = n₁) :
  ...
  · intro hf
    ...
    · aesop_cat
  · rintro ⟨X, f, h, mem, _⟩
    obtain ⟨e, he⟩ := t.triangle_iso_exists n₀ n₁ (by omega) _ _
...
instance (X : C) (a b : ℤ) [t.IsLE X b] : t.IsLE ((t.truncLE a).obj X) b := by
  by_cases h : a ≤ b
  · exact {le := t.le_monotone h _ (t.le_of_isLE _ _)}
  · simp only [not_le] at h
    have : t.IsLE X a := {le := t.le_monotone (le_of_lt h) _ (t.le_of_isLE _ _)}
    refine {le := ?_}
    exact (t.le b).prop_of_iso (show X ≅ _ from (asIso ((t.truncLEι a).app X)).symm)
      (t.le_of_isLE _ _)

instance (X : C) (a b : ℤ) [t.IsLE X b] : t.IsLE ((t.truncLT a).obj X) b :=
  {le := (t.le b).prop_of_iso ((t.truncLEIsoTruncLT (a-1) a (by omega)).app X) (t.le_of_isLE _ _)}

instance (X : C) (a b : ℤ) [t.IsGE X a] : t.IsGE ((t.truncGE b).obj X) a := by
  by_cases h : a ≤ b
  · exact {ge := t.ge_antitone h _ (t.ge_of_isGE _ _)}
  · simp only [not_le] at h
    have : t.IsGE X b := {ge := t.ge_antitone (le_of_lt h) _ (t.ge_of_isGE _ _)}
    exact {ge := (t.ge a).prop_of_iso (show X ≅ _ from asIso ((t.truncGEπ b).app X))
                 (t.ge_of_isGE _ _)}

instance (X : C) (a b : ℤ) [t.IsGE X a] : t.IsGE ((t.truncGT b).obj X) a :=
  {ge := (t.ge a).prop_of_iso ((t.truncGTIsoTruncGE b (b+1) (by omega)).symm.app X)
         (t.ge_of_isGE _ _)}
...
lemma isIso₁_truncLT_map_of_GE (T : Triangle C) (hT : T ∈ distTriang C)
  ...
  rw [← NatIso.isIso_map_iff (t.truncLEIsoTruncLT (n-1) n (by omega))]
  exact t.isIso₁_truncLE_map_of_GE T hT (n-1) n (by omega) h₃
...
lemma isIso₂_truncGE_map_of_LE (T : Triangle C) (hT : T ∈ distTriang C)
  ...
  have : t.IsLE (X⟦(1 : ℤ)⟧) (n₀-1) := t.isLE₂ _ H.mem (n₀-1)
    ...
    (by refine {le := ?_}
        change (t.le _).shift _ _
        rw [t.shift_le _ _ n₀ (by omega)]
        have : t.IsLE ((t.triangleLEGE n₀ n₁ h).obj T.obj₃).obj₁ n₀ := by
          dsimp; infer_instance
        exact t.le_of_isLE _ _ )
  refine {le := ?_}
  rw [← t.shift_le 1 (n₀ - 1) n₀ (by omega)]
...
instance (X : C) (a b : ℤ) [t.IsGE X a] : t.IsGE ((t.truncLE b).obj X) a := by
  rw [t.isGE_iff_isZero_truncLE_obj (a-1) a (by omega)]
  have := t.isIso₁_truncLE_map_of_GE _ ((t.triangleLEGE_distinguished b (b+1) rfl X))
    (a-1) a (by omega) (by dsimp ; infer_instance)
  dsimp at this
  exact IsZero.of_iso (t.isZero_truncLE_obj_of_isGE (a-1) a (by omega) X)
    (asIso ((t.truncLE (a - 1)).map ((t.truncLEι b).app X)))

instance (X : C) (a b : ℤ) [t.IsGE X a] : t.IsGE ((t.truncLT b).obj X) a :=
  {ge := (t.ge a).prop_of_iso ((t.truncLEIsoTruncLT (b-1) b (by omega)).app X) (t.ge_of_isGE _ _)}

instance (X : C) (a b : ℤ) [t.IsLE X b] : t.IsLE ((t.truncGE a).obj X) b := by
  rw [t.isLE_iff_isZero_truncGE_obj b (b+1) rfl]
  have := t.isIso₂_truncGE_map_of_LE _ ((t.triangleLEGE_distinguished (a-1) a (by omega) X))
...
lemma isZero_truncGEt_obj_obj (X : C) (n : ℤ) [t.IsLE X n] (j : ℤt) (hj : ℤt.mk n < j) :
  ...
  · simp at hj
    ...
    exact t.isLE_of_LE X n (j - 1) (by omega)
  · apply Functor.zero_obj
...
lemma truncLTt_obj_obj_isLE (n : ℤ) (i : ℤt) (h : i ≤ ℤt.mk (n + 1)) (X : C) :
  ...
  · simp at h
    dsimp
    exact t.isLE_of_LE _ (i - 1) n (by omega)
  · simp at h
-/
...
instance (X : C) (n : ℤ) : t.IsLE ((t.homology'' n).obj X) 0 := by
  refine {le := ?_}
  change (t.le _).shift _ _
  rw [t.shift_le _ _ n (by omega)]
  exact t.le_of_isLE _ _

instance (X : C) (n : ℤ) : t.IsGE ((t.homology'' n).obj X) 0 := by
  refine {ge := ?_}
  change (t.ge _).shift _ _
...
lemma isIso_truncGE_map_truncGEπ_app (a b : ℤ) (h : a ≤ b) (X : C) :
  ...
  have : t.IsLE ((t.triangleLEGE (a - 1) a (by omega)).obj X).obj₁ (a - 1) := by
    dsimp; infer_instance
  t.isIso₂_truncGE_map_of_LE _
    (t.triangleLEGE_distinguished (a-1) a (by omega) X) (b-1) b (by omega)
      (by exact {le := t.le_monotone (a := a - 1) (b := b - 1) (by omega) _ (t.le_of_isLE _ _)})

lemma isIso_truncLT_map_truncLTι_app (a b : ℤ) (h : a ≤ b) (X : C) :
    IsIso ((t.truncLT a).map ((t.truncLTι b).app X)) :=
  have : t.IsGE ((t.triangleLTGE b).obj X).obj₃ b := by dsimp; infer_instance
  t.isIso₁_truncLT_map_of_GE _ (t.triangleLTGE_distinguished b X) a
  (by exact {ge := t.ge_antitone h _ (t.ge_of_isGE _ _)})
...
lemma isIso_truncLTι_app_truncGELT_obj (a b : ℤt) (h : a ≤ b) (X : C) :
  ...
  · dsimp [SpectralObject.AbstractSpectralObject.truncLTι]
    simp only [comp_id]
    rw [← t.isLE_iff_isIso_truncLTι_app (b-1) b (by omega)]
    obtain (rfl|⟨a, rfl⟩|rfl) := a.three_cases
    · dsimp
      infer_instance
    · dsimp
...
noncomputable def natTransTruncGELTTruncLTGE (a b : ℤ) :
  ...
  app X := t.liftTruncLT (t.descTruncGE
    ((t.truncLTι b).app X ≫ (t.truncGEπ a).app X) a) (b-1) b (by omega)
  naturality X Y f := by
...
instance (a b : ℤ) (X : C) : IsIso ((t.natTransTruncGELTTruncLTGE a b).app X) := by
  ...
  · let u₁₂ := (t.natTransTruncLTOfLE a b h).app X
    ...
    dsimp at this
    have eq' : t.liftTruncLT m₁ (b-1) b (by omega) =
    ...
    have fac : (t.truncLTι b).app ((t.truncGE a).obj ((t.truncLT b).obj X)) ≫
        t.liftTruncLT m₁ (b-1) b (by omega) = (t.truncLT b).map m₁ :=
  ...
  · refine ⟨0, ?_, ?_⟩
    all_goals
      apply IsZero.eq_of_src
      exact t.isZero _ (b-1) a (by omega)
...
/-noncomputable def shiftSpectralObjectω₁IsoHomologyιHeart' (X : C) (q q' : ℤ) (hq' : q + 1 = q') :
    ((t.spectralObject X).ω₁ ⋙ shiftFunctor C q).obj
      (ComposableArrows.mk₁ (homOfLE (by simp; omega) : ℤt.mk q ⟶ ℤt.mk q')) ≅
...
lemma HasInducedTStructure.mk' (S : Subcategory C) (t : TStructure C)
  ...
  exists_triangle_zero_one X hX := by
    ...
    · exact TStructure.le_of_isLE  _ _ _
    · exact TStructure.ge_of_isGE  _ _ _

open ObjectProperty

lemma mem_of_hasInductedTStructure (S : Subcategory C) (t : TStructure C)
    [IsClosedUnderIsomorphisms S.P] [S.HasInducedTStructure t]
  ...
  obtain ⟨e, _⟩ := t.triangle_iso_exists n₀ n₁ (by omega) _ _ hT
    ...
    (Iso.refl _) inferInstance inferInstance (by
      ...
      infer_instance)
  exact ⟨(prop_iff_of_iso S.P (Triangle.π₁.mapIso e)).2 (S.ι_obj_mem _),
    (prop_iff_of_iso S.P (Triangle.π₃.mapIso e)).2 (S.ι_obj_mem _)⟩

instance (S S' : Subcategory C) (t : TStructure C) [S.HasInducedTStructure t]
    [S'.HasInducedTStructure t]
    [IsClosedUnderIsomorphisms S.P] [IsClosedUnderIsomorphisms S'.P] :
